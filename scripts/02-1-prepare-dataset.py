...
out_path = os.path.join(data_path, 'dataset-1')
...
num_near_samples_per_polygon = 5
num_far_samples_per_polygon = 5
...
for img_folder in range(1, 11):
    ...
    img, mask, annot = create_mask(data_path, img_folder, annot_round=1)[:3]
...
with open(os.path.join(out_path, 'splits-1.json'), 'w') as out_f:
